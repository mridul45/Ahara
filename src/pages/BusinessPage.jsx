...
/* ================= Lightweight localStorage persister ================= */
...
// Hook up persistence (keep cache up to 1 day)
persistQueryClient({
  queryClient,
...
/* ================= API (ETag-aware) ================= */
// You provided this exact URL (no trailing slash)
const FEATURED_URL_PRIMARY = 'https://ahara-be.onrender.com/api/content/playlist/featured';
// Fallback tries with trailing slash (covers DRF strict routers/redirects)
...
async function fetchFeaturedPlaylists() {
  ...
  // 1) Try the exact URL you shared
  let res = await doFetch(FEATURED_URL_PRIMARY, etag);

  // If 404/308/301/302, try a second request with trailing slash
  ...
  // 304 → use whatever we already have
  if (res.status === 304) {
  ...
  // Be forgiving on 404: show empty (but log once)
  if (res.status === 404) {
...
function BusinessPageInner() {
  ...
  // Show capsule when the set of playlist IDs changes
  useEffect(() => {
